...
def geometry(b):
    ...
    depth=pow(floor_area/max(b.aspect_ratio,1e-9),0.5)
...
def compute_degradation_index(day_idx,t_amb,rh,severity_mult,maint_factor,dcfg):
    if dcfg.degradation_model=="Linear time-series":
        ...
        base=1.0-pow(2.718281828,-dcfg.exponential_rate_per_day*(day_idx+1))
    else:
        time_term=1.0-pow(2.718281828,-dcfg.fouling_growth_B*0.01*(day_idx+1))
